...
class UpsActivity : AppCompatActivity() {

    private lateinit var activity: Activity
    private lateinit var imageViewFon2: ImageView
    private lateinit var up: List<Up>

    private lateinit var rv_table: RecyclerView
    private lateinit var adapter: UpAdapter
    ...
    private fun loadData() {
        GlobalScope.launch(Dispatchers.IO) {
            try {
                val data = URL("http://135.181.248.237/12/ups.json").readText()
                val gson = Gson()
                up = gson.fromJson(data, Array<Up>::class.java).toList()

                activity.runOnUiThread {
                    adapter = UpAdapter(up)
